...
CommandRunner::Handler* CurrentUserController::removeFriends(const std::string &profileId) {
...
CommandRunner::Handler* CurrentUserController::addFriends(const std::string &profileId) {
    auto cmd = RequestCommand::createWithCommand("account/password");
    CC_SAFE_AUTORELEASE(cmd);
    cmd->setValue("/id", profileId);
